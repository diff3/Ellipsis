...
LibClassicDurations:Register(Ellipsis) -- tell library it's being used and should start working
...
function Ellipsis:UNIT_AURA(unitTag)
	...
	if spellID then
	  durationNew, expirationTimeNew = LibClassicDurations:GetAuraDurationByUnit(unitTag, spellID, unitCaster, spellName)
	end

	if duration == 0 and durationNew then
	  duration = durationNew
	  expireTime = expirationTimeNew
	end
			...
			if (((filterByBlacklist and not blacklist[spellID]) or (not filterByBlacklist and whitelist[spellID])) and
				...
				if (noTargetRedirect[spellID]) then -- spell needs to be redirected to notarget unit
					...
					if (not unit) then -- got this far, we'll (soon) have an aura to add, make a unit to place it on
						...
						local override	= (guid == targetGUID) and 'target' or false
